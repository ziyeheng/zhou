			...
			<div class="crumb-wrap">
				<div class="crumb-list">
					<i class="icon-font"></i><a href="<%=basePath%>yjgetYJ.action">首页</a><span
						class="crumb-step">&gt;</span><span class="crumb-name">抢到使用券记录</span>
				</div>
			</div>
						...
						<table class="search-tab">
							<tr>
								...
								<th width="80">
									...
									name="yjbtn1" value="查询" type="button" onclick="chaxuncoupon2()"></td>
							</tr>
						</table>
					</form>
				</div>
			...
			<div class="result-wrap">
				<form name="myform" id="myform" method="post">
					...
					<div class="result-content">
						...
						<div class="list-page">
							<a href="<%=basePath%>regetcoupon2.action?nowpage=1">首页</a>
							<c:if test="${nowpage>1}">
								<a
									href="<%=basePath %>regetcoupon2.action?nowpage=${nowpage-1 }">上一页</a>
							</c:if>
							<c:if test="${nowpage==1}">
								<a href="<%=basePath %>regetcoupon2.action?nowpage=${nowpage}">上一页</a>
							</c:if>
							第${nowpage }页，共${totalpage }页
							<c:if test="${nowpage<totalpage}">
								<a
									href="<%=basePath %>regetcoupon2.action?nowpage=${nowpage+1 }">下一页</a>
							</c:if>
							<c:if test="${nowpage==totalpage}">
								<a href="<%=basePath %>regetcoupon2.action?nowpage=${nowpage }">下一页</a>
							</c:if>
							<a href="<%=basePath %>regetcoupon2.action?nowpage=${totalpage }">尾页</a>
						</div>
					</div>
				</form>
			</div>
