			...
			<div class="crumb-wrap">
				<div class="crumb-list">
					<i class="icon-font"></i><a href="<%=basePath%>yjgetYJ.action">首页</a><span
						class="crumb-step">&gt;</span><span class="crumb-name">抢到红包记录</span>
				</div>
			</div>
						...
						<table class="search-tab">
							<tr>
								...
								<td><input class="btn btn-primary btn2" id="yjbtn1"
									name="yjbtn1" value="查询" type="button" onclick="chaxunredmoney()"></td>
							</tr>
						</table>
					</form>
				</div>
				...
				<form name="myform" id="myform" method="post">
					...
					<div class="result-content">
						<table class="result-tab" width="100%">
							...
							<c:forEach items="${redmoneylist}" var="uobj">
								<tr>
									...
									<td width="50px">${uobj.price}</td>
									<td width="100px">${uobj.mobel}</td>
									<td width="100px">${uobj.phone}</td>
									<td width="70px">${uobj.yongjin}</td>
									<td width="100px">${uobj.add_time}</td>

								</tr>
							</c:forEach>
						</table>
						<div class="list-page">
							<a href="<%=basePath%>regetredmoney.action?nowpage=1">首页</a>
							<c:if test="${nowpage>1}">
								<a href="<%=basePath %>regetredmoney.action?nowpage=${nowpage-1 }">上一页</a>
							</c:if>
							<c:if test="${nowpage==1}">
								<a href="<%=basePath %>regetredmoney.action?nowpage=${nowpage}">上一页</a>
							</c:if>
							第${nowpage }页，共${totalpage }页
							<c:if test="${nowpage<totalpage}">
								<a href="<%=basePath %>regetredmoney.action?nowpage=${nowpage+1 }">下一页</a>
							</c:if>
							<c:if test="${nowpage==totalpage}">
								<a href="<%=basePath %>regetredmoney.action?nowpage=${nowpage }">下一页</a>
							</c:if>
							<a href="<%=basePath %>regetredmoney.action?nowpage=${totalpage }">尾页</a>
						</div>
					</div>
				</form>
			</div>
